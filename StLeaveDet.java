...
public class StLeaveDet extends JFrame implements ActionListener{
	Choice croll;
	JTable tb;
	...
	StLeaveDet(){
		...
		add(croll);
		String query = "select * from stuleave;";
		String query1 = "select distinct(roll) from stuleave;";
	...
	public void actionPerformed(ActionEvent e) {
		try{
			...
			setVisible(false);
				...
				Connection con = DriverManager.getConnection(url,user,pwd);
				String query = "select * from stuleave where roll = ?;";
				PreparedStatement ps = con.prepareStatement(query);
